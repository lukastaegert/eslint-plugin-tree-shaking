...
const reportSideEffectsInProgram = (context, programNode) => {
  ...
  const reportSideEffectsInCallToReference = options => variableReference => {
    ...
    if (writeExpr) {
      switch (writeExpr.type) {
        ...
        case 'ArrowFunctionExpression':
          ...
          break
        default:
          context.report(writeExpr, `Expression with unknown side-effects is called as a function`)
      }
    }
  }

  const reportSideEffectsInCallToDefinition = (scope, options) => variableDefinition => {
  ...
  const reportSideEffectsInAssignmentToMemberExpression = (node, scope, options) => {
  ...
  const REPORT_EFFECTS_BY_TYPE = {
    ...
    CallExpression(node, scope, options) {
    ...
    ExportDefaultDeclaration(node, scope, options) {
      reportSideEffects(node.declaration, scope, options)
    },
    ...
    IfStatement(node, scope, options){
    ...
    NewExpression(node, scope, options) {
      reportSideEffectsInCallExpression(node, scope,
