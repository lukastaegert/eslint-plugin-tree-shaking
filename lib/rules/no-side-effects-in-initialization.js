...
const {
  getChildScopeForNodeIfExists, getLocalVariable, getRootNode, isPureFunction
...
const reportSideEffectsInProgram = (context, programNode) => {
  ...
  const NODES = {
    ...
    FunctionDeclaration: {
      reportEffectsWhenCalled(node, scope, options){
        ...
        if (!functionScope) {
          ...
          reportSideEffects(node.body, functionScope,
    ...
    MemberExpression: {
      ...
      reportEffectsWhenCalled(node, scope, options){
        ...
        if (rootNode.type !== 'Identifier' || getLocalVariable(rootNode.name, scope) ||
          !isPureFunction(node)) {
    ...
    UnaryExpression: {
      getValueAndReportEffects(node, scope, options) {
        ...
        return getValueAndReportSideEffects(node.argument, scope, options)
  ...
  const verifyDefinitionTypeIsKnown = definition => {
    if (!DEFINITIONS[definition.type]) {
      reportFatalError(node, `Unexpected definition target ${definition.type}.`)
    }
  }
  ...
  function reportSideEffectsInDefinitionWhenCalled (scope, options) {
    return definition => {
      ...
      if (DEFINITIONS[definition.type].reportEffectsWhenCalled) {
        DEFINITIONS[definition.type].reportEffectsWhenCalled(definition,
          scope, options)
      }
    }
  }
  ...
  function reportSideEffectsInDefinitionWhenMutated (scope, options) {
    return definition => {
      verifyDefinitionTypeIsKnown(definition)
      if (DEFINITIONS[definition.type].reportSideEffectsWhenMutated) {
        DEFINITIONS[definition.type].reportSideEffectsWhenMutated(definition,
          scope, options)
      }
    }
  }
  ...
  if (!moduleScope) {
    reportFatalError(node, 'Could not find module scope.')
  } else {
