...
const reportSideEffectsInProgram = (context, programNode) => {
  ...
  const IS_ROLLUP = context.options[0] && context.options[0].compatibility === 'rollup'
  ...
  const NODES = {
    ...
    ConditionalExpression: {
      ...
      reportEffectsWhenCalled (node, scope, options) {
        const testResult = getValueAndReportSideEffects(node.test, scope, options)
        if (IS_ROLLUP) {
          reportRollup(node, 'Calling a conditional expression is always a side-effect')
    ...
    LogicalExpression: {
      getValueAndReportEffects (node, scope, options) {
        if (IS_ROLLUP) {
          const leftValue = getValueAndReportSideEffects(node.left, scope, options)
          const rightValue = getValueAndReportSideEffects(node.right, scope, options)
          return LOGICAL_OPERATORS[node.operator](() => leftValue, () => rightValue)
        }
      ...
      reportEffectsAsExpressionStatement (node, scope, options) {
        reportRollup(node, 'LogicalExpressions used as statements are never removed')
        reportSideEffects(node, scope, options)
      }
    },
    ...
    SequenceExpression: {
      getValueAndReportEffects (node, scope, options) {
        if (IS_ROLLUP) {
          node.expressions.forEach(expression => reportSideEffects(expression, scope, options))
          return Value.unknown()
        }
      ...
      reportEffectsAsExpressionStatement (node, scope, options) {
        reportRollup(node, 'SequenceExpression used as statements are never removed')
        reportSideEffects(node, scope, options)
      }
    },
  ...
  if (!moduleScope) {
    ...
    programNode.body.forEach(subNode => {
      if (IS_ROLLUP && subNode.type === 'BlockStatement' && subNode.body.length > 0) {
        reportRollup(subNode, 'Top-level non-empty BlockStatements are not cleaned up properly')
      }
