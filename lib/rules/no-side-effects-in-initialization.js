...
const getUnknownSideEffectError = subject =>
  `Cannot determine side-effects of ${subject}`

const getAssignmentError = target =>
  getUnknownSideEffectError(`assignment to ${target}`)
const getCallError = target => getUnknownSideEffectError(`calling ${target}`)
const getExpressionStatementError = target =>
  getUnknownSideEffectError(`executing ${target} as a statement`)
const getMutationError = target =>
  getUnknownSideEffectError(`mutating ${target}`)
...
const ERROR_CALL_SUPER = getCallError('super class constructor')
const ERROR_DEBUGGER = 'Debugger statements are side-effects'
const ERROR_DELETE_OTHER = getUnknownSideEffectError(
  'deleting anything but a MemberExpression'
)
...
const reportSideEffectsInProgram = (context, programNode) => {
  ...
  const NODES = {
    ...
    ArrowFunctionExpression: {
      ...
      reportEffectsWhenMutated (node) {
        reportRollup(node, 'Mutating an ArrowFunction is a side-effect')
      }
    },
    ...
    AssignmentExpression: {
      ...
      reportEffectsAsExpressionStatement (node, scope, options) {
    ...
    ClassExpression: {
      reportEffects (node, scope, options) {
        reportRollup(node, 'ClassExpressions are never removed')
        node.superClass && reportSideEffects(node.superClass, scope, options)
        reportSideEffects(node.body, scope, options)
      },
      reportEffectsWhenCalled (node, scope, options) {
    ...
    ConditionalExpression: {
      ...
      reportEffectsWhenCalled (node, scope, options) {
        ...
        if (IS_ROLLUP) {
          reportRollup(
            node,
            'Calling a conditional expression is always a side-effect'
          )
    ...
    ContinueStatement: {
      reportEffects (node) {
        reportRollup(node, 'ContinueStatements are never removed')
      }
    },
    ...
    ExpressionStatement: {
      reportEffects (node, scope, options) {
        if (IS_ROLLUP) {
          reportSideEffectsAsExpressionStatement(
            node.expression,
            scope,
            Object.assign({}, options, {
              hasValidThis:
    ...
    IfStatement: {
      reportEffects (node, scope, options) {
        ...
        if (testResult.hasValue) {
          console.log('if value', testResult.value)
          testResult.value
            ? reportSideEffects(node.consequent, scope, options)
            : node.alternate &&
              reportSideEffects(node.alternate, scope, options)
    ...
    JSXAttribute: {
      reportEffects (node, scope, options) {
        node.value && reportSideEffects(node.value, scope, options)
      }
    },
    ...
    JSXOpeningElement: {
      reportEffects (node, scope, options) {
        ...
        node.attributes.forEach(subNode =>
          reportSideEffects(subNode, scope, options)
        )
      }
    },
    ...
    LogicalExpression: {
      ...
      reportEffectsAsExpressionStatement (node, scope, options) {
        reportRollup(
          node,
          'LogicalExpressions used as statements are never removed'
        )
        reportSideEffects(node, scope, options)
      }
    },
    ...
    MetaProperty: {
      reportEffects (node) {
        reportRollup(node, 'MetaProperties are never removed')
      }
    },
    ...
    SequenceExpression: {
      ...
      reportEffectsAsExpressionStatement (node, scope, options) {
        reportRollup(
          node,
          'SequenceExpression used as statements are never removed'
        )
        reportSideEffects(node, scope, options)
      }
    },
    ...
    Super: {
      ...
      reportEffectsWhenCalled (node, scope, options) {
        if (options.superClass) {
          ...
          context.report(node, ERROR_CALL_SUPER)
        }
      }
    },
    ...
    YieldExpression: {
      reportEffects (node, scope, options) {
        reportRollup(node, 'YieldExpressions are never removed')
        node.argument && reportSideEffects(node.argument, scope, options)
      },
      reportEffectsAsExpressionStatement (node, scope, options) {
        reportRollup(node, 'YieldExpressions are never removed')
  ...
  function reportSideEffectsAsExpressionStatement (node, scope, options) {
    if (!verifyNodeTypeIsKnown(node)) {
      return
    }
    if (NODES[node.type].reportEffectsAsExpressionStatement) {
      NODES[node.type].reportEffectsAsExpressionStatement(node, scope, options)
    } else {
      context.report(node, getExpressionStatementError(node.type))
    }
  }
