...
const reportSideEffectsInProgram = (context, programNode) => {
  ...
  const NODES = {
    ...
    ExpressionStatement: {
      reportEffects (node, scope, options) {
        reportSideEffectsAsExpressionStatement(node.expression, scope, options)
      }
    },
    ...
    ObjectExpression: {
      reportEffects (node, scope, options) {
        node.properties.forEach(subNode => {
          subNode.key && reportSideEffects(subNode.key, scope, options)
          subNode.value && reportSideEffects(subNode.value, scope, options)
        })
      },
      reportEffectsWhenMutated: noEffects
    },
    ...
    Super: {
      reportEffects (node, scope, options) {
        reportRollup(node, 'Super is never removed')
      },
      reportEffectsWhenCalled (node, scope, options) {
        reportRollup(node, 'Super is never removed')
